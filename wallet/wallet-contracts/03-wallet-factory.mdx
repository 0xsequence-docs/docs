...
<Warning> Unsanitized input <br/>
The Factory contract **does not** check if the provided `_mainModule` is a valid Sequence wallet implementation, or if `_salt` is a valid Sequence configuration.<br/>
Using invalid parameters will result on a proxy contract that **may not** be usable, and could result in loss of funds.</Warning>
...
This property alongside the use of [SingletonDeployer](/wallet/wallet-contracts/universal-deployer) or [UniversalDeployer](/wallet/wallet-contracts/universal-deployer) is the reason why Sequence wallets can obtain the same address on multiple chains.

<Tip> Gas-free wallet creation <br/>
Sequence wallets are **100% counterfactual**, meaning that funds can be safely transferred to the wallet's address without the need for calling the `deploy` function.<br/>
Deploying the wallet is only required before the first transaction is sent **from** the wallet.</Tip>
